  ...
  return withAuth(req, async (request, userId) => {
    try {
      ...
      if (keywords.length > 50) {
        return NextResponse.json({ error: '한 번에 최대 50개까지 키워드를 추가할 수 있습니다.' }, { status: 400 })
      }

      // 사용자의 블로그 프로젝트 찾기 (1개만 존재)
      const project = await prisma.blogTrackingProject.findFirst({
        where: {
          userId: userId
        }
      })
