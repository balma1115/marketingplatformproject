...
import { ImprovedNaverScraperV3WithCallbacks } from '@/lib/services/improved-scraper-v3-with-callbacks'
  ...
  return withAuth(req, async (request, userId) => {
    ...
    const processTracking = async () => {
      try {
        ...
        // 개선된 스크래퍼 V3 with callbacks 사용
        const scraper = new ImprovedNaverScraperV3WithCallbacks()
        console.log('Using Improved V3 scraper with real-time callbacks')
        const playwrightCrawler = new PlaywrightCrawlerService()
        
        // 한국 시간(KST) 기준으로 날짜 설정
        ...
        console.log(`Starting to track ${keywords.length} keywords with real-time updates`)
        ...
        send({ 
          ...
          current: 0, 
          total: keywords.length, 
          keyword: '동시 추적 시작 (최대 3개 동시 처리)...',
          activeTracking: 0
        })
        
        // Queue로 모든 키워드 동시 처리 (실시간 콜백 포함)
        const resultsPromise = scraper.trackMultipleKeywordsWithCallbacks(
          keywordData, 
          {
          ...
          {
            onStart: (keyword, index) => {
              activeTracking++
              console.log(`Started tracking: ${keyword} (${activeTracking} active)`)
              send({ 
                type: 'progress', 
                status: 'tracking',
            ...
            onComplete: async (keyword, index, total, result) => {
              activeTracking--
              ...
              try {
                ...
                console.log(`Saved ranking for ${keyword}: organic=${result.organicRank}, ad=${result.adRank} (${activeTracking} active)`)
                successCount++
                ...
                console.error(`Failed to save ranking for ${keyword}:`, error)
                failCount++
                send({ 
                  type: 'warning', 
                  current: successCount + failCount, 
                  total: keywords.length, 
                  keyword: keyword,
                  message: `${keyword} 저장 실패`,
                  activeTracking: activeTracking
                })
              }
            },
            onError: async (keyword, index, total, error) => {
              activeTracking--
