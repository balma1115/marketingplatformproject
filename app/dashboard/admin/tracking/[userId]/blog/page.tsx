...
import { ArrowLeft, Plus, Search, TrendingUp, BarChart, Calendar, Eye, EyeOff, Trash2, RefreshCw, ExternalLink } from 'lucide-react'
        ...
        <div className="mb-6">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex space-x-2">
              <button
                onClick={() => router.push(`/blog/keywords/trend/${keywords[0]?.id}?userId=${userId}`)}
              ...
              <button
                onClick={() => router.push(`/blog/keywords/monthly?userId=${userId}`)}
          ...
          <table className="w-full">
            <thead className="bg-gray-50 border-b">
              <tr>
                ...
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">통합검색</th>
                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">블로그탭</th>
              ...
              {keywords.map((keyword) => (
                <tr key={keyword.id} className={!keyword.isActive ? 'bg-gray-50' : ''}>
                  ...
                  <td className="px-4 py-3">
                    {keyword.blogTabRank ? (
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${
                        keyword.blogTabRank <= 5 ? 'bg-green-100 text-green-800' :
                        keyword.blogTabRank <= 10 ? 'bg-yellow-100 text-yellow-800' :
                        keyword.blogTabRank <= 20 ? 'bg-blue-100 text-blue-800' :
                        'bg-gray-100 text-gray-800'
                      }`}>
                        {keyword.blogTabRank}위
                      </span>
                    ) : (
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center space-x-2">
                      <button
                        onClick={() => router.push(`/blog/keywords/trend/${keyword.id}?userId=${userId}`)}
