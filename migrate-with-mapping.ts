...
async function migrateWithMapping() {
  try {
    console.log('=== 데이터 마이그레이션 시작 (사용자 매핑 포함) ===\n')
    ...
    for (const sqliteUser of sqliteUsers) {
      ...
      if (pgUser) {
        ...
        console.log(`User mapping: SQLite ${sqliteUser.id} (${sqliteUser.email}) -> PostgreSQL ${pgUser.id}`)
      }
    }
    ...
    for (const project of blogProjects) {
      ...
      if (!pgUserId) {
        console.log(`  ⚠️ 사용자 매핑 없음: user_id ${project.user_id}`)
        continue
      }
      ...
      if (!existing) {
        try {
          ...
          for (const keyword of keywords) {
            ...
            // 블로그 결과 마이그레이션 (최근 10개만)
            const results = db.prepare('SELECT * FROM blog_tracking_results WHERE keyword_id = ? ORDER BY tracking_date DESC LIMIT 10').all(keyword.id) as any[]
    ...
    for (const sp of smartplaces) {
      ...
      if (!pgUserId) {
        console.log(`  ⚠️ 사용자 매핑 없음: user_id ${sp.user_id}`)
        continue
      }
      ...
      if (!existing) {
        try {
          ...
          for (const keyword of keywords) {
            ...
            // 스마트플레이스 순위 마이그레이션 (최근 30개만)
            const rankings = db.prepare('SELECT * FROM smartplace_rankings WHERE keyword_id = ? ORDER BY check_date DESC LIMIT 30').all(keyword.id) as any[]
